...
public:
	ObjID_t				GetID( )const{ return m_ObjID; }	//获取角色ID
	VOID				SetID( ObjID_t id ){ m_ObjID = id; }	//设置角色ID
	virtual UINT		GetUniqueID(VOID) const;	//获取唯一ID
...
public:
	...
	const WORLD_POS*	getWorldPos()const{ return &m_Pos; }				//获取角色世界坐标
	VOID				setWorldPos( const WORLD_POS* pos ){ m_Pos = *pos; }//设置角色世界坐标
	virtual VOID		Teleport( const WORLD_POS* pos );			//瞬移
...
public:
	...
	BOOL				UpdateZone( VOID );		//更新区域数据

private:
	ObjID_t					m_ObjID;			//对象ID
	UINT					m_uPoolID;			//对象池ID
