...
BOOL ODBCInterface::Execute()
{
	_MY_TRY
	{
		...
		SQLRowCount(hStmt,(long int*)&mAffectCount);
...
BOOL	ODBCInterface::Fetch()
{
	...
	for (int ColIndex=0;ColIndex<mColCount;ColIndex++)
	{
		...
		int				Ret		= 0;
		while(Ret=SQLGetData(hStmt, ColIndex+1, SQL_C_CHAR,Col[ColIndex]+TotalGet,MAX_COLUMN_BUFFER,(long int*)&LenData)!=SQL_NO_DATA)
		{
			break;
		}
...
BOOL ODBCInterface::LongFetch()
{
	...
	for (int ColIndex=0;ColIndex<mColCount;ColIndex++)
	{
		...
		* 严重注意！！！严重注意！！！严重注意！！！严重注意！！！
		*/
		while(Ret=SQLGetData(hStmt, ColIndex+1, SQL_C_CHAR,Col[ColIndex]+TotalGet,MAX_LONG_COLUMN_BUFFER,(long int*)&LenData)!=SQL_NO_DATA)
		{
			break;
		}
		//printf("sql length=%d ret=%d,LenData=%d\r\n",TotalGet,Ret,LenData);
